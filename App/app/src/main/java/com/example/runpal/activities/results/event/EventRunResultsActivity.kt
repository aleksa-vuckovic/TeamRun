...
class EventRunResultsActivity : ComponentActivity() {
    ...
    val vm: EventRunResultsViewModel by viewModels()
                    ...
                    Scaffold(
                        topBar = {
                            StandardTopBar(
                                ...
                                onRefresh =  vm::reload,
                                onAccount = {startActivity(Intent(this@EventRunResultsActivity, AccountActivity::class.java))},
                                onLogout = {
                                    loginManager.logout()
                                    this@EventRunResultsActivity.restartApp()
                                },
                                title = stringResource(id = curDestination.title!!),
                                counterTitle = { if (vm.state == EventRunResultsViewModel.State.LOADED) StandardBadge(text = DateOnlyFormatter.format(vm.data.time).first, type = BadgeType.CLASSIC) }
                            )
                        },
                        bottomBar = {
                        ...
                        if (vm.state == EventRunResultsViewModel.State.LOADING) LoadingScreen()
                        else if (vm.state == EventRunResultsViewModel.State.ERROR) ErrorScreen(message = "Try again.")
